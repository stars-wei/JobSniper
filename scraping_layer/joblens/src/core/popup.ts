...
document.addEventListener('DOMContentLoaded', async function() {
	...
	try {
		...
		if (refreshButton) {
            ...
            const fullAutoButton = document.getElementById("full-auto");
            if (fullAutoButton) {
                fullAutoButton.addEventListener("click", async (e) => {
                    e.preventDefault();
                    try {
                        await browser.runtime.sendMessage({
                            action: "sendMessageToTab",
                            tabId: currentTabId!,
                            message: { action: "auto-scroll-and-save" }
                        });
                        window.close();
                    } catch (error) {
                        console.error("Full-auto failed:", error);
                    }
                });
            }
            ...
            if (autoScrollButton) {
                autoScrollButton.addEventListener("click", async (e) => {
                    console.log("DEBUG: Auto-scroll button clicked, sending message to tab:", currentTabId);
                    e.preventDefault();
                    autoScrollButton.classList.add("loading");
                    try {
                        await browser.runtime.sendMessage({
                            action: "sendMessageToTab",
                            tabId: currentTabId!,
                            message: { action: "auto-scroll" }
                        });
                        // Refresh fields to capture newly loaded content
                        await refreshFields(currentTabId!, { checkTemplateTriggers: false });
                    } catch (error) {
                        console.error("Auto-scroll failed:", error);
                    } finally {
                        autoScrollButton.classList.remove("loading");
                    }
                });
            }
